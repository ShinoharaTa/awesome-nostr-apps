...
**アクセス制御（ACL）**: 下記コマンドは `shinoemon.acl.smartHome` に列挙した npub / hex からの
投稿だけに反応します。**空配列・未設定なら全員拒否**（誰にも反応しない）です。許可外の投稿には
返信も SwitchBot API 呼び出しもしません（拒否は警告ログのみ）。読み取り（まいへや / 光ある？）と
操作（光あれ）は同じ許可リストで扱います。

| トリガー | 条件 | 応答 |
| --- | --- | --- |
| `^まいへや$` | ACL 許可済み & 温湿度計が取得できる | 自宅の温度・湿度を返す |
| `^光ある？$` | ACL 許可済み & ライト状態が取得できる | 指定ライトごとの点灯状況を返す |
| `^光あれ[！!…]?$` | ACL 許可済み & `allowControl: true` | 指定ライトを点灯し、成功時 `光あれ` |

上記コマンドは、Nostr クライアントが本文に付ける `nostr:npub...` / `nostr:nprofile...`
メンションがあっても取り除いて判定します。署名は購読時（SimplePool）に検証済みのため無効署名は
届きません。加えて「光あれ」は古い投稿（既定 180 秒より前）を操作として実行しません。
